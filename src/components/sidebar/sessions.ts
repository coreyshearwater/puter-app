import { AppState as IAppState, Session, ChatMessage } from '../../types.js';
...
export function initializeSessions() {
    ...
    if (!AppState.sessions || AppState.sessions.length === 0) {
        ...
        const initialMessages: ChatMessage[] = AppState.messages.length > 0 ? [...AppState.messages] : [];
...
function renderSessionList() {
    ...
    if (!list) {
        console.error('❌ session-list not found in DOM');
        return;
    }
