...
function setupSettingsListeners() {
    ...
    if (themeSelector) {
        themeSelector.onchange = (e) => applyTheme((e.target as HTMLSelectElement).value);
    }
    
    // --- NEW TOGGLE LOGIC ---
    const setupToggle = (id: string, initialState: boolean, onToggle: (checked: boolean) => void) => {
    ...
    if (tempSlider) {
        ...
        tempSlider.onchange = () => saveStateToKV();
    }

    const maxTokensInput = document.getElementById('max-tokens-input') as HTMLInputElement;
    if (maxTokensInput) {
        maxTokensInput.onchange = (e) => {
            AppState.maxTokens = parseInt((e.target as HTMLInputElement).value);
            saveStateToKV();
        };
    }
    
    const btnExport = document.getElementById('btn-export-chat');
    if (btnExport) btnExport.onclick = () => window.gravityChat.exportChat();
    
    const btnClear = document.getElementById('btn-clear-chat');
    if (btnClear) btnClear.onclick = () => window.gravityChat.clearChat();
    
    const btnBrowse = document.getElementById('btn-browse-voices');
    if (btnBrowse) btnBrowse.onclick = () => window.gravityChat.openVoiceBrowser();
    ...
    if (ssoInput && ssoRwInput) {
        const saveGrokCookies = () => {
            AppState.grokCookies = {
                sso: (ssoInput as HTMLInputElement).value.trim(),
                'sso-rw': (ssoRwInput as HTMLInputElement).value.trim()
            };
            saveStateToKV();
        };

        (ssoInput as HTMLInputElement).oninput = saveGrokCookies;
        (ssoRwInput as HTMLInputElement).oninput = saveGrokCookies;
        ...
        if (magicPasteBtn) {
            magicPasteBtn.onclick = async () => {
                try {
                    ...
                    let found = false;
                    if (ssoMatch) {
                        (ssoInput as HTMLInputElement).value = ssoMatch[1];
                        found = true;
                    }
                    if (ssoRwMatch) {
                        (ssoRwInput as HTMLInputElement).value = ssoRwMatch[1];
                        found = true;
                    }
