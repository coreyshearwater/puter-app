...
const POLLY_VOICES: Voice[] = [
    ...
    { id: 'Ivy', name: 'Ivy', gender: 'Female', lang: 'en-US', accent: 'Child' },
    { id: 'Justin', name: 'Justin', gender: 'Male', lang: 'en-US', accent: 'Child' },
    { id: 'Kevin', name: 'Kevin', gender: 'Male', lang: 'en-US', accent: 'Child' },
    // UK English
...
let edgeVoicesCache: Voice[] | null = null;
let previewAudio: HTMLAudioElement | null = null;

async function fetchEdgeVoices(): Promise<Voice[] | null> {
...
function getNativeVoices(): Voice[] {
...
let previousVoice: string | null = null; // For cancel/revert

function selectVoice(voiceId: string, displayName: string) {
    ...
    const sel = document.getElementById('voice-selector') as HTMLSelectElement;
...
// State
let activeTab: 'edge' | 'polly' | 'native' = 'edge';
let searchQuery = '';
let langFilter = '';
...
function renderVoiceList() {
    ...
    const filterBar = document.getElementById('vb-lang-filter') as HTMLSelectElement;
    if (filterBar) {
        ...
        return `
            <div class="vb-voice-card ${isSelected ? 'vb-selected' : ''}" data-id="${v.id}">
                ...
                <div class="flex items-center gap-1 flex-shrink-0">
                    ...
                    <button class="vb-btn-preview" onclick="event.stopPropagation(); (window as any)._vbPreview('${v.id}')" title="Preview">
...
export async function openVoiceBrowser() {
    ...
    previousVoice = AppState.selectedVoice || null; 
    ...
    modal.innerHTML = `
        <div class="glass-card w-full max-w-xl slide-in relative" onclick="event.stopPropagation()" style="max-height: 80vh; display: flex; flex-direction: column;">
            ...
            <div class="flex justify-between items-center p-6 pb-4 relative z-10">
                ...
                <button onclick="(window as any)._vbClose()" class="p-2 text-gray-400 hover:text-white transition-colors">
    ...
    // Wire globals
    (window as any)._vbPreview = previewVoice;
    (window as any)._vbClose = closeVoiceBrowser;
    ...
    // Wire search
    const searchEl = document.getElementById('vb-search') as HTMLInputElement;
    searchEl.addEventListener('input', (e: any) => {
        searchQuery = e.target.value;
        renderVoiceList();
    });

    // Wire lang filter
    document.getElementById('vb-lang-filter')?.addEventListener('change', (e: any) => {
