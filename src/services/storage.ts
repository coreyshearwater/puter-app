...
export async function saveStateToKV() {
    ...
    const stateSnapshot: Partial<IAppState> = {
    ...
    saveTimeout = setTimeout(async () => {
        try {
            await (window as any).puter.kv.set('gravitychat_personas', JSON.stringify(AppState.personas));
            await (window as any).puter.kv.set('gravitychat_active_persona', AppState.activePersona?.id || null);
            await (window as any).puter.kv.set('gravitychat_settings', JSON.stringify(stateSnapshot));
...
export async function loadStateFromKV() {
    ...
    try {
        ...
        const settingsData = await (window as any).puter.kv.get('gravitychat_settings');
        if (settingsData) {
            const settings = JSON.parse(settingsData);
            ...
            for (const key of SAFE_KEYS) {
                ...
                if (value !== undefined) (AppState as any)[key] = value;
            }
        ...
        const personasData = await (window as any).puter.kv.get('gravitychat_personas');
        if (personasData) AppState.personas = JSON.parse(personasData);
        
        const activePersonaId = await (window as any).puter.kv.get('gravitychat_active_persona');
        if (activePersonaId && activePersonaId !== 'null') {
            AppState.activePersona = AppState.personas.find(p => p.id === activePersonaId) || null;
        } else {
