...
import { loadVoices as loadEngineVoices, speakText as engineSpeakText, captureAudio, stopHardwareMic } from './voice/engine.js';
import { handleQueueSpeech, stopAllSpeech, transcribeAndAction } from './voice/logic.js';
...
export async function queueSpeech(text: string, bubble: HTMLElement | null) {
    return handleQueueSpeech(text, bubble, startRecording);
}

export async function speakText(text: string, bubble: HTMLElement | null, isInternal: boolean = false) {
