...
import { selectModel } from '../components/sidebar/models.js';
...
export async function processSemanticCommand(transcription: string): Promise<boolean> {
    ...
    try {
        ...
        const responsePromise = await puter.ai.chat([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: `Command: ${commandText}` }
        ], { model: INTENT_MODEL, temperature: 0, stream: false });

        const data = JSON.parse(responsePromise.message.content);
