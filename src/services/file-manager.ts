...
export async function loadFiles(path: string): Promise<void> {
    ...
    try {
        ...
        if (typeof (window as any).puter === 'undefined' || !(await (window as any).puter.auth.isSignedIn())) {
        ...
        const entries: FSEntry[] = await (window as any).puter.fs.readdir(path);
        ...
        if (list) list.innerHTML = `<div class="text-center p-4 text-xs text-red-400">Error: ${error?.message || 'Unknown error'}</div>`;
    }
}
...
export async function createNewFile() {
    ...
    try {
        ...
        await (window as any).puter.fs.write(fullPath, '');
...
export async function createNewFolder() {
    ...
    try {
        const fullPath = `${AppState.currentPath}/${name}`.replace(/\/+/g, '/');
        await (window as any).puter.fs.mkdir(fullPath);
        loadFiles(AppState.currentPath);
...
export async function deleteFile(path: string) {
    ...
    try {
        await (window as any).puter.fs.delete(path);
        loadFiles(AppState.currentPath);
    } catch (e) { showToast('Delete failed', 'error'); }
}

export async function previewFile(path: string, attach: boolean = false) {
    try {
        let content: any = await (window as any).puter.fs.read(path);
        ...
        if (attach) {
            ...
            if (typeof (window as any).puter !== 'undefined' && (window as any).puter.ui && (window as any).puter.ui.createWindow) {
                const fileName = path.split('/').pop();
                await (window as any).puter.ui.createWindow({
...
export async function updateStorageMeter() {
    ...
    try {
        ...
        if (typeof (window as any).puter === 'undefined') return;
        ...
        if (typeof (window as any).puter.fs.space !== 'function') {
            container.classList.add('hidden'); // Hide if API doesn't exist
            return;
        }
        
        const space = await (window as any).puter.fs.space();
...
export async function selectDirectory() {
    try {
        const dir = await (window as any).puter.ui.showDirectoryPicker();
        if (dir && dir.path) {
            loadFiles(dir.path);
