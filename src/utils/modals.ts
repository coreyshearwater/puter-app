...
export function showErrorModal(title: string, message: string) {
    ...
    modal.innerHTML = `
        <div class="flex items-start justify-between mb-4">
            ...
            <button class="btn btn-sm btn-square btn-ghost text-gray-500 hover:text-white" id="close-modal-x">×</button>
        </div>
    ...
    const xBtn = backdrop.querySelector('#close-modal-x') as HTMLElement;
    const dismissBtn = backdrop.querySelector('#close-modal-btn') as HTMLElement;
    if (xBtn) xBtn.onclick = closeHandler;
    ...
    const escHandler = (e: KeyboardEvent) => { if (e.key === 'Escape') { closeHandler(); document.removeEventListener('keydown', escHandler); } };
    document.addEventListener('keydown', escHandler);

    return { close: closeHandler };
}

export function showCustomPersonaModal(existingName: string | null, existingPrompt: string | null, onSave: (name: string, prompt: string) => void) {
    // Backdrop
    const backdrop = document.createElement('div');
    backdrop.className = 'fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 slide-in';
    ...
    // Safe text injection
    const titleEl = modal.querySelector('#cp-title');
    if (titleEl) titleEl.textContent = 'Custom Persona';

    const nameInput = modal.querySelector('#cp-name') as HTMLInputElement;
    const promptInput = modal.querySelector('#cp-prompt') as HTMLTextAreaElement;
    if (existingName && nameInput) nameInput.value = existingName;
    if (existingPrompt && promptInput) promptInput.value = existingPrompt;
    ...
    const saveHandler = () => {
        const nameVal = (document.getElementById('cp-name') as HTMLInputElement)?.value.trim() || 'Custom';
        const promptVal = (document.getElementById('cp-prompt') as HTMLTextAreaElement)?.value.trim();
        if (!promptVal) {
            showToast('System prompt is required', 'error');
            return;
        }
        onSave(nameVal, promptVal);
        closeHandler();
    };

    const xBtn = backdrop.querySelector('#close-cp-x') as HTMLElement;
    const cancelBtn = backdrop.querySelector('#close-cp-btn') as HTMLElement;
    const saveBtn = backdrop.querySelector('#save-cp-btn') as HTMLElement;
    if (xBtn) xBtn.onclick = closeHandler;
...
export function showConfirmModal(title: string, message: string, onConfirm: () => void) {
    ...
    const cancelBtn = backdrop.querySelector('#confirm-cancel') as HTMLElement;
    const okBtn = backdrop.querySelector('#confirm-ok') as HTMLElement;
    if (cancelBtn) cancelBtn.onclick = closeHandler;
...
export function showInfoModal(title: string, message: string) {
    ...
    const msgEl = modal.querySelector('#info-message');
    if (titleEl) titleEl.textContent = title;
    if (msgEl) msgEl.textContent = message;
    ...
    const xBtn = backdrop.querySelector('#close-info-x') as HTMLElement;
    const closeBtn = backdrop.querySelector('#close-info-btn') as HTMLElement;
    if (xBtn) xBtn.onclick = closeHandler;
